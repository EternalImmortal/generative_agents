...
class Persona:
    ...
    def perceive(self, maze):
        ...
        该函数获取当前迷宫，并返回角色周围发生的事件。 重要的是，感知由角色的两个关键超参数引导：1) att_bandwidth，2) 保留。
        首先，<att_bandwidth> 确定角色可以感知的附近事件的数量。 假设有 10 个事件在人物角色的视野范围内——感知所有 10 个事件可能会太多。
        因此，角色会感知最接近的 att_bandwidth 事件数量，以防事件太多。
        其次，角色不想在每个时间步骤感知和思考同一事件。
        这就是 <retention> 的用武之地——人物角色的记忆是有时间顺序的。
        因此，如果角色的记忆包含最近一次保留中发生的当前周围事件，则无需再次感知。 xx
