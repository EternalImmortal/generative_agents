...
class ReverieServer:
    ...
    def start_server(self, int_counter):
        ...
        intervals.
        遐想的主后端服务器。此函数将环境文件从前端检索到
        了解世界状况，呼吁每个角色基于世界状态的决策，并在某个步骤保存他们的动作间隔。
        INPUT
        ...
        # 用于追踪和game_obj_cleanup相关的特定event
        ...
        while (True):
            ...
            # <curr_env_file> 文件是我们前端输出的文件。
            # 当前端完成其工作并移动角色后，它将放置一个与我们的步数相匹配的新环境文件。
            # 那就是我们运行这个 for 循环的内容的时候。 否则，我们就只能等待。
            curr_env_file = f"{sim_folder}/environment/{self.step}.json"
            if check_if_file_exists(curr_env_file):
                # If we have an environment file, it means we have a new perception
                # input to our personas. So we first retrieve it.
                # 如果我们有一个环境文件，这意味着我们有一个新的感知输入到我们的角色中。 所以我们首先检索它。
                try:
                ...
                if env_retrieved:
                    ...
                    # 在新的周期里，清理game_obj_cleanup中的所有对象动作。
                    ...
                    for persona_name, persona in self.personas.items():
                        ...
                        # 现在，角色将前往目的地。 *一旦*角色到达那里，我们就激活对象动作。
                    ...
                    # 然后我们需要真正让每个角色感知并移动。
                    # 每个角色的移动以角色将移动的 x y 坐标的形式出现。
                    # 例如，(50, 34) 这是调用角色的核心大脑的地方。
...
if __name__ == '__main__':
    ...
    target = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + '_the_ville_isabella_maria_klaus'
